...
const LectureTable = () => {
  ...
  const [pageSize, setPageSize] = useState(getPagination('calendar') || 10);
  ...
  const handlePageSizeChange = (event) => {
    ...
    setPagination('calendar', event.target.value);
  };
          ...
          >
            ...
            {(batches || []).map((b, i) => (
              <MenuItem key={i} value={b._id}>
                {`${b.month.slice(0, 3).toUpperCase()}${b.year}- ${b.courseCode}-${b.batchNo}`}
              </MenuItem>
            ))}
          </Select>
        </FormControl>
